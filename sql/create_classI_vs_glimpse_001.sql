CREATE TABLE classI_vs_glimpse_001_corrected AS 
...
FROM gt1 AS gt JOIN gl_w49 AS gl 
WHERE (gl.ra > (gt.ra - 0.001)) AND (gl.ra < (gt.ra + 0.001)) AND (gl.dec > (gt.dec - 0.001)) AND (gl.dec < (gt.dec + 0.001))
